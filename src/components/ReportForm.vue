<script setup lang="ts">
import { reactive, ref } from 'vue'

import SectionTitle from './SectionTitle.vue'
import BaseInputField from './BaseInputField.vue'
import BaseButton from './BaseButton.vue'
import CurrencyInputField from './CurrencyInputField.vue'

const isGenerating = ref(false)

const form = reactive({
  pageSize: 'A4',
  title: '',
  description: '',
  amount: 0,
})

const errors = reactive({
  pageSize: '',
  title: '',
  description: '',
  amount: '',
})

const validateForm = () => {
  errors.pageSize = form.pageSize ? '' : 'Page size harus diisi'

  if (!form.title) {
    errors.title = 'Judul laporan harus diisi'
  } else if (form.title.length < 5) {
    errors.title = 'Judul laporan minimal 5 karakter'
  } else {
    errors.title = ''
  }

  if (!form.description) {
    errors.description = 'Deskripsi harus diisi'
  } else if (form.description.length < 10) {
    errors.description = 'Deskripsi minimal 10 karakter'
  } else {
    errors.description = ''
  }

  if (form.amount === 0 || form.amount === null) {
    errors.amount = 'Nominal harus diisi'
  } else if (Number(form.amount) < 0 || Number.isNaN(Number(form.amount))) {
    errors.amount = 'Nominal minimal Rp 0'
  } else {
    errors.amount = ''
  }

  return !Object.values(errors).some(Boolean)
}

const handleGenerate = () => {
  if (!validateForm()) return

  isGenerating.value = true

  setTimeout(() => {
    emit('generated', {
      pageSize: form.pageSize,
      title: form.title,
      amount: form.amount,
      createdAt: new Date().toISOString(),
    })

    isGenerating.value = false
    form.pageSize = 'A4'
    form.title = ''
    form.description = ''
    form.amount = 0
  }, 1500)
}

const emit = defineEmits(['generated'])
</script>

<template>
  <section
    class="flex flex-col gap-y-4 2xl:gap-y-6 p-6 2xl:p-8 bg-white rounded-lg 2xl:rounded-xl shadow"
  >
    <SectionTitle text="Formulir Laporan" />

    <form @submit.prevent="handleGenerate" class="flex flex-col gap-y-4 2xl:gap-y-6">
      <div class="flex flex-col md:flex-row gap-4 2xl:gap-6">
        <div class="shrink-0 flex flex-col gap-y-2">
          <label for="page-size" class="font-medium">
            Ukuran Halaman
            <span class="text-red-500">*</span>
          </label>

          <select
            id="page-size"
            v-model="form.pageSize"
            :disabled="isGenerating"
            class="px-3 py-2 border border-gray-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 rounded-md transition"
            :class="
              errors.pageSize
                ? 'border-red-500 focus-visible:ring-red-500'
                : 'focus-visible:ring-blue-500/75'
            "
          >
            <option value="A4">A4</option>
            <option value="A5">A5</option>
            <option value="Letter">Letter</option>
          </select>

          <p v-if="errors.pageSize" class="text-sm text-red-500">
            {{ errors.pageSize }}
          </p>
        </div>

        <BaseInputField
          id="title"
          label="Judul Laporan"
          placeholder="Masukkan judul laporan"
          :disabled="isGenerating"
          v-model="form.title"
          :error="errors.title"
          required
        />
      </div>

      <BaseInputField
        type="textarea"
        id="description"
        label="Deskripsi / Isi Laporan"
        :rows="4"
        placeholder="Masukkan deskripsi / isi laporan"
        :disabled="isGenerating"
        v-model="form.description"
        :error="errors.description"
        required
      />

      <CurrencyInputField
        id="amount"
        label="Nominal (Rp)"
        placeholder="Masukkan nominal"
        :disabled="isGenerating"
        v-model="form.amount"
        :error="errors.amount"
        required
      />

      <div class="flex justify-end">
        <BaseButton type="submit" text="Generate PDF" :loading="isGenerating">
          <template #icon>
            <svg
              class="size-5 2xl:size-7"
              viewBox="0 0 16 16"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M14 4.5V14C14 15.1046 13.1046 16 12 16H11V15H12C12.5523 15 13 14.5523 13 14V4.5H11C10.1716 4.5 9.5 3.82843 9.5 3V1H4C3.44772 1 3 1.44772 3 2V11H2V2C2 0.89543 2.89543 0 4 0H9.5L14 4.5ZM1.59961 11.85H0V15.849H0.791016V14.5072H1.59375C1.88086 14.5072 2.125 14.4496 2.32617 14.3344C2.5293 14.2172 2.68359 14.059 2.78906 13.8598C2.89648 13.6586 2.9502 13.433 2.9502 13.183C2.9502 12.933 2.89746 12.7074 2.79199 12.5063C2.68652 12.3051 2.5332 12.1459 2.33203 12.0287C2.13281 11.9096 1.88867 11.85 1.59961 11.85ZM2.14453 13.183C2.14453 13.3315 2.11621 13.4584 2.05957 13.5639C2.00488 13.6674 1.92578 13.7475 1.82227 13.8041C1.71875 13.8588 1.59375 13.8861 1.44727 13.8861H0.788086V12.4799H1.44727C1.66602 12.4799 1.83691 12.5404 1.95996 12.6615C2.08301 12.7826 2.14453 12.9565 2.14453 13.183ZM3.36211 11.85V15.849H4.82109C5.22344 15.849 5.55645 15.7699 5.82012 15.6117C6.08574 15.4535 6.28398 15.224 6.41484 14.9232C6.5457 14.6225 6.61113 14.2611 6.61113 13.8393C6.61113 13.4193 6.5457 13.0609 6.41484 12.7641C6.28594 12.4672 6.08965 12.2406 5.82598 12.0844C5.5623 11.9281 5.22734 11.85 4.82109 11.85H3.36211ZM4.15312 12.4945H4.71562C4.96367 12.4945 5.1668 12.5453 5.325 12.6469C5.48516 12.7484 5.60332 12.8998 5.67949 13.101C5.75762 13.3022 5.79668 13.5531 5.79668 13.8539C5.79668 14.0805 5.77422 14.2777 5.7293 14.4457C5.68633 14.6137 5.6209 14.7543 5.53301 14.8676C5.44707 14.9789 5.33574 15.0629 5.19902 15.1195C5.0623 15.1742 4.90117 15.2016 4.71562 15.2016H4.15312V12.4945ZM7.89609 14.2582V15.849H7.10508V11.85H9.65391V12.5033H7.89609V13.6195H9.50156V14.2582H7.89609Z"
                fill="currentcolor"
              />
            </svg>
          </template>
        </BaseButton>
      </div>
    </form>
  </section>
</template>
